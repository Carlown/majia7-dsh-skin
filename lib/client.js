window.__ModuleLoader__.load({id:"majia7-skin",factory:(require)=>{var module={exports:{}};var exports=module.exports;
...
function indicatorLabel(rawFrame, efforts) {
  ...
  return effort === void 0 ? liangRankForFrame(rawFrame) : effort.name;
}
...
function installLiangAppearanceButton(scope, presenter) {
  ...
  const sync = () => {
    ...
    if (!snapshot.enabled) {
      ...
      const bindingText = document.documentElement.lang.toLowerCase().startsWith("en") ? "Bind \u9A6C\u52A0\u4E03\u76AE\u80A4 slider to reasoning level" : "\u9A6C\u52A0\u4E03\u76AE\u80A4 \u7ED1\u5B9A\u601D\u8003\u7B49\u7EA7";
...
function createPreferenceStore() {
  ...
  const onStorage = (event) => {
    if (event.key !== BIND_EFFORT_KEY && event.key !== BACKDROP_DEPTH_KEY && event.key !== PERSONA_KEY) return;
    const next = {
